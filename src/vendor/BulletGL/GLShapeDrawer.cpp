...
void GLShapeDrawer::drawCylinder(float radius, float halfHeight, int upAxis)
{
  ...
  gluCylinder(quadObj, radius, radius, 2.0 * halfHeight, 15, 10);
  glTranslatef(0.0, 0.0, 2.0 * halfHeight);
  glRotatef(-180.0, 0.0, 1.0, 0.0);
  gluDisk(quadObj, 0.0, radius, 15, 10);
...
void renderSquareA(float x, float y, float z)
{
  ...
  glVertex3f(x + 10.0, y + 10.0, z);
  glVertex3f(x, y + 10.0, z);
  glEnd();
}
...
void GLShapeDrawer::drawOpenGL(btScalar* m, btCollisionShape* shape, const btVector3& color, int  debugMode, const btVector3& worldBoundsMin, const btVector3& worldBoundsMax)
{
  ...
  if (shape->getShapeType() == UNIFORM_SCALING_SHAPE_PROXYTYPE) {
    btUniformScalingShape* scalingShape = static_cast<btUniformScalingShape*>(shape);
    btConvexShape* convexShape = scalingShape->getChildShape();
